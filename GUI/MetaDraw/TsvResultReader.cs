...
namespace MetaMorpheusGUI
{
    public class TsvResultReader
    {
        ...
        public static List<MetaDrawPsm> ReadTsv(string filePath)
        {
            ...
            while (reader.Peek() > 0)
            {
                ...
                try
                {
                    ...
                    List<MatchedFragmentIon> peaks = ReadFragmentIonsFromString(matchedPeaks, peptideSequence);
        ...
        {
            // TODO: might get messed up on ambiguous peptides "|"
            var theoreticalPeptide = new PeptideWithSetModifications(peptideFullSequence, GlobalVariables.AllModsKnownDictionary);
            var peaks = matchedMzString.Split(mzSplit, StringSplitOptions.RemoveEmptyEntries).Select(v => v.Trim());

            // TODO: figure out the dissociation type from the annotated fragments
            // use a custom fragmentation type and just add the different fragment types as annotated
            var theoreticalProducts = theoreticalPeptide.Fragment(DissociationType.HCD, FragmentationTerminus.Both);
            ...
            foreach (var peak in peaks)
            {
                ...
                var t = new NeutralTerminusFragment(FragmentationTerminus.None, 0, 0, 0);
                Product p = new Product(ProductType.a, t, 0);
                matchedIons.Add(new MatchedFragmentIon(p, double.NaN, 0, 0));
            }
