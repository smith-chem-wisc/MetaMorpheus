...
namespace OldInternalLogic
{
	public class PeptideWithSetModifications : Peptide
	{
		...
		static readonly double waterMonoisotopicMass = PeriodicTable.GetElement("H").PrincipalIsotope.AtomicMass * 2 + PeriodicTable.GetElement("O").PrincipalIsotope.AtomicMass;

		double monoisotopicMass = double.NaN;

		string extendedSequence;

		string sequence;

		PeptideWithPossibleModifications modPep;
