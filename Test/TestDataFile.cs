...
namespace Test
{
    internal class TestDataFile : MsDataFile<TestScan>
    {
        ...
        public TestDataFile() : base(null)
        {
            ...
            Scans.Add(new TestScan(2, 2, MassSpectrum2, 402.18629720155.ToMz(2), 2, 1, 402.18629720155.ToMz(2), 2, 1));
        }

        public TestDataFile(List<PeptideWithSetModifications> pepWithSetModss) : base(null)
        {
            ...
            for (int i = 0; i < pepWithSetModss.Count; i++)
            {
                ...
                Scans.Add(new TestScan(2 * i + 2, 2 * i + 1, MassSpectrum2, pepWithSetMods.MonoisotopicMass.ToMz(2), 2, 1, pepWithSetMods.MonoisotopicMass.ToMz(2), 2 * i + 1, 2 * i + 1));
            }
        }

        public TestDataFile(PeptideWithSetModifications pepWithSetMods) : base(null)
        {
            var mz1 = new double[] { pepWithSetMods.MonoisotopicMass.ToMz(2), (pepWithSetMods.MonoisotopicMass + 1.003).ToMz(2), (pepWithSetMods.MonoisotopicMass + 2.005).ToMz(2) };
            var intensities1 = new double[] { 1, 1, 1 };
            var MassSpectrum1 = new MzmlMzSpectrum(mz1, intensities1, false);
            ...
            var MassSpectrum2 = new MzmlMzSpectrum(mz2.OrderBy(b => b).ToArray(), intensities2.ToArray(), false);
            Scans.Add(new TestScan(2, 2, MassSpectrum2, pepWithSetMods.MonoisotopicMass.ToMz(2), 2, 1, pepWithSetMods.MonoisotopicMass.ToMz(2), 2, 1));
        }
