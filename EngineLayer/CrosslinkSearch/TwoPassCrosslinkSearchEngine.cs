...
namespace EngineLayer.CrosslinkSearch
{
    public class TwoPassCrosslinkSearchEngine : MetaMorpheusEngine
    {
        ...
        protected override MetaMorpheusEngineResults RunSpecific()
        {
            ...
            Parallel.ForEach(Partitioner.Create(0, ListOfSortedms2Scans.Length), new ParallelOptions { MaxDegreeOfParallelism = commonParameters.MaxThreadsToUsePerFile }, (range, loopState) =>
            {
                ...
                for (int i = range.Item1; i < range.Item2; i++)
                {
                    ...
                    // first-pass scoring
                    IndexedScoring(allBinsToSearch, scoringTable, byteScoreCutoff, idsOfPeptidesPossiblyObserved, scan.PrecursorMass, FragmentIndex);
        ...
        private void IndexedScoring(List<int> binsToSearch, byte[] scoringTable, byte byteScoreCutoff, List<int> idsOfPeptidesPossiblyObserved, double scanPrecursorMass, List<int>[] theFragmentIndex)
        {
            ...
            for (int i = 0; i < binsToSearch.Count; i++)
            {
                ...
                for (int j = 0; j <= peptideIdsInThisBin.Count - 1; j++)
                {
