...
namespace EngineLayer
{
    public class ProteinParsimonyEngine : MetaMorpheusEngine
    {
        ...
        private const double PeptidePEPvalueCutoffForParsimony = 0.3;
        ...
        public ProteinParsimonyEngine(List<PeptideSpectralMatch> allPsms, bool modPeptidesAreDifferent, CommonParameters commonParameters, List<(string fileName, CommonParameters fileSpecificParameters)> fileSpecificParameters, List<string> nestedIds) : base(commonParameters, fileSpecificParameters, nestedIds)
        {
            ...
            {
                _fdrFilteredPsms = allPsms.Where(p => p.FullSequence != null && p.FdrInfo.PEP <= PeptidePEPvalueCutoffForParsimony).ToList();
            }
            else
            {
                _fdrFilteredPsms = allPsms.Where(p => p.BaseSequence != null && p.FdrInfo.PEP <= PeptidePEPvalueCutoffForParsimony).ToList();
            }
