...
namespace EngineLayer.Analysis
{
    public class AnalysisEngine : MetaMorpheusEngine
    {
        ...
        private readonly Action<List<NewPsmWithFdr>, List<ProteinGroup>, string, List<string>> writeMzIdentmlAction;
            ...
            for (int j = 0; j < searchModes.Count; j++)
            {
                if (newPsms[j] != null)
                {
                    ...
                    if (doParsimony && writeProteinGroupsAction != null)
                    {
                        ...
                        writeMzIdentmlAction(orderedPsmsWithFDR, proteinGroups[j], searchModes[j].FileNameAddition, nestedIds);
                    }
                    ...
                    if (arrayOfSortedMS2Scans == null && doParsimony)
                    {
                        ...
                        foreach (var file in fileNames)
                        {
                            ...
                            writeMzIdentmlAction(psmsGroupedByFilename.Where(p => p.Key == file).SelectMany(g => g).ToList() , subsetProteinGroupsForThisFile, searchModes[j].FileNameAddition, nestedIds);
                        }
                    }
