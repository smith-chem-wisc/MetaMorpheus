...
namespace mzCal
{
    public static class SoftwareLockMassRunner
    {
        ...
        {
            p.OnOutput("Welcome to my software lock mass implementation");
            p.OnOutput("Calibrating " + Path.GetFileName(p.myMsDataFile.FilePath));

            List<int> trainingPointCounts = new List<int>();
            List<LabeledDataPoint> pointList;
            for (int calibrationRound = 1; ; calibrationRound++)
            {
                p.OnOutput("Calibration round " + calibrationRound);

                p.OnOutput("Getting Training Points");
                ...
                {
                    p.OnOutput("Not enough MS1 training points, identification quality is poor");
                    return;
                }
                WriteDataToFiles(pointList1, "pointList1" + p.myMsDataFile.Name + calibrationRound);
                p.OnOutput("pointList1.Count() = " + pointList1.Count());
                var pointList2 = pointList.Where((b) => b.inputs[0] == 2).ToList();
                if (pointList2.Count == 0)
                {
                    p.OnOutput("Not enough MS2 training points, identification quality is poor");
                    return;
                }
                WriteDataToFiles(pointList2, "pointList2" + p.myMsDataFile.Name + calibrationRound);
                p.OnOutput("pointList2.Count() = " + pointList2.Count());

                CalibrationFunction identityPredictor = new IdentityCalibrationFunction(p.OnOutput);
                p.OnOutput("Uncalibrated MSE, " + identityPredictor.getMSE(pointList1) + "," + identityPredictor.getMSE(pointList2) + "," + identityPredictor.getMSE(pointList));
            ...
            p.OnOutput("Post-processing");

            p.postProcessing(p);

            p.OnOutput("Finished running my software lock mass implementation");
            p.OnProgress(0);
        }
        ...
        private static CalibrationFunction Calibrate(List<LabeledDataPoint> trainingPoints, SoftwareLockMassParams p)
        {
            ...
            p.OnOutput("Uncalibrated MSE, " + bestMS1MSE + "," + bestMS2MSE + "," + combinedMSE);
        ...
        private static void CalibrateSpectra(SoftwareLockMassParams p, CalibrationFunction bestCf)
        {
            foreach (var a in p.myMsDataFile)
            {
                if (a.MsnOrder == 2)
                {
                    ...
                    {
                        p.OnWatch("Calibrating scan number " + a.OneBasedScanNumber);
                        p.OnWatch(" before calibration:");
                        p.OnWatch(" " + string.Join(",", a.MassSpectrum.newSpectrumExtract(p.mzRange).xArray));
                    }
                    ...
                    if (p.MS2spectraToWatch.Contains(a.OneBasedScanNumber))
                    {
                        p.OnWatch(" after calibration:");
                        p.OnWatch(" precursorMZ:" + precursorMZ);
                        p.OnWatch(" monoisotopicMZ:" + monoisotopicMZ);
                        p.OnWatch(" newSelectedMZ:" + newSelectedMZ);
                        p.OnWatch(" newMonoisotopicMZ:" + newMonoisotopicMZ);
                        p.OnWatch(" " + string.Join(",", a.MassSpectrum.newSpectrumExtract(p.mzRange).xArray));
                    }
                }
                else
