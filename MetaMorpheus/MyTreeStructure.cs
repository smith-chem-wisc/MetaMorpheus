...
namespace MetaMorpheus
{
    public class MyTreeStructure
    {
        ...
        public void GenerateBins(IEnumerable<PeptideSpectrumMatch> targetMatches, double dc)
        {
            this.dc = dc;
            List<double> listOfMassShifts = targetMatches.Select(b => b.PrecursorMassErrorDa).OrderBy(b => b).ToList();
        ...
        public void AddToBins(IEnumerable<PeptideSpectrumMatch> enumerable)
        {
            foreach (var ok in enumerable)
                foreach (Bin bin in finalBins)
                    if (Math.Abs(ok.PrecursorMassErrorDa - bin.MassShift) <= dc)
                        bin.Add(ok);
        }
    }
}