...
namespace Test
{
    ...
    public static class ParameterTest
    {
        ...
        public static void TestFileSpecifcParameterOverwrite()
        {
            ...
            Assert.That(defaultParameters.DissociationType, !Is.EqualTo(notDefaultParameters.DissociationType));
            Assert.That(defaultParameters.DoPrecursorDeconvolution, !Is.EqualTo(notDefaultParameters.DoPrecursorDeconvolution));
            Assert.That(defaultParameters.UseProvidedPrecursorInfo, !Is.EqualTo(notDefaultParameters.UseProvidedPrecursorInfo));
            Assert.That(defaultParameters.DeconvolutionIntensityRatio, !Is.EqualTo(notDefaultParameters.DeconvolutionIntensityRatio));
            Assert.That(defaultParameters.DeconvolutionMaxAssumedChargeState, !Is.EqualTo(notDefaultParameters.DeconvolutionMaxAssumedChargeState));
            Assert.That(defaultParameters.ReportAllAmbiguity, !Is.EqualTo(notDefaultParameters.ReportAllAmbiguity));
            Assert.That(defaultParameters.AddCompIons, !Is.EqualTo(notDefaultParameters.AddCompIons));
            Assert.That(defaultParameters.TotalPartitions, !Is.EqualTo(notDefaultParameters.TotalPartitions));
            Assert.That(defaultParameters.ScoreCutoff, !Is.EqualTo(notDefaultParameters.ScoreCutoff));
            Assert.That(defaultParameters.NumberOfPeaksToKeepPerWindow, !Is.EqualTo(notDefaultParameters.NumberOfPeaksToKeepPerWindow));
            Assert.That(defaultParameters.MinimumAllowedIntensityRatioToBasePeak, !Is.EqualTo(notDefaultParameters.MinimumAllowedIntensityRatioToBasePeak));
            Assert.That(defaultParameters.TrimMs1Peaks, !Is.EqualTo(notDefaultParameters.TrimMs1Peaks));
            Assert.That(defaultParameters.TrimMsMsPeaks, !Is.EqualTo(notDefaultParameters.TrimMsMsPeaks));
            Assert.That(defaultParameters.ProductMassTolerance, !Is.EqualTo(notDefaultParameters.ProductMassTolerance));
            Assert.That(defaultParameters.PrecursorMassTolerance, !Is.EqualTo(notDefaultParameters.PrecursorMassTolerance));
            Assert.That(defaultParameters.DeconvolutionMassTolerance, !Is.EqualTo(notDefaultParameters.DeconvolutionMassTolerance));
            Assert.That(defaultParameters.MaxThreadsToUsePerFile, !Is.EqualTo(notDefaultParameters.MaxThreadsToUsePerFile));
            Assert.That(defaultParameters.DigestionParams, !Is.EqualTo(notDefaultParameters.DigestionParams));
            Assert.That(defaultParameters.ListOfModsVariable, !Is.EqualTo(notDefaultParameters.ListOfModsVariable));
            Assert.That(defaultParameters.ListOfModsFixed, !Is.EqualTo(notDefaultParameters.ListOfModsFixed));
            Assert.That(defaultParameters.CustomIons, !Is.EqualTo(notDefaultParameters.CustomIons));
