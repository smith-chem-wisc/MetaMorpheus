...
namespace Test
{
    internal class NonSpecificSearchEngineTests
    {
        ...
        [Test]
        public void TestSnesIndexedScoringExceedUpperBound()
        {
            SearchParameters SearchParameters = new SearchParameters
            {
                SearchTarget = true,
            ...
            //adding this big fragment mass to exceed the upper bound in the Snes Method
            Product bigProduct = new Product(ProductType.b, FragmentationTerminus.Both, 360000, 1, 1, 0, ProductType.b, 2);
            fragments.Add(bigProduct);

            var fragmentMzArray = fragments.Select(x => x.NeutralMass.ToMz(1)).ToArray();

            var myMsDataFile = new TestDataFile(guiltyPwsm.MonoisotopicMass, fragmentMzArray);
            var variableModifications = new List<Modification>();
            var fixedModifications = new List<Modification>();
            ModificationMotif.TryGetMotif("C", out ModificationMotif motif2);
            Modification mod2 = new Modification(_originalId: "Carbamidomethyl on C", _modificationType: "Common Fixed", _target: motif2, _locationRestriction: "Anywhere.", _monoisotopicMass: 57.02146372068994);
