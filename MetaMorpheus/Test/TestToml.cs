    ...
    public static class TestToml
    {
        ...
        public static void TestFileSpecificDeconvolutionParams()
        {
            ...
            try
            {
                ...
                // ===== UniDec citation in the manuscript prose =====
                //
                // UniDec's licence requires software redistributing IsoDec to tell the end user it is in
                // use and pass on the citation request, so this is a licence obligation rather than a
                // nicety. The four runs above are exactly the cases that matter: the citation must follow
                // what actually ran, not what the task was configured with, and two of these four have a
                // file-specific toml that disagrees with the task.
                static string Prose(string dir, string run) =>
                    File.ReadAllText(Path.Combine(dir, run, "AutoGeneratedManuscriptProse.txt"));

                Assert.Multiple(() =>
                {
                    Assert.That(Prose(outputDir, "IsoDecDirect"), Does.Contain("10.1021/acs.analchem.5b00140"),
                        "IsoDec set on the task: the prose must cite UniDec");
                    Assert.That(Prose(outputDir, "IsoDecFromToml"), Does.Contain("10.1021/acs.analchem.5b00140"),
                        "IsoDec reached only through a file-specific toml: the prose must still cite UniDec");
                    Assert.That(Prose(outputDir, "IsoDecDirect"), Does.Contain("UniDec"),
                        "the citation has to name UniDec, which is what the licence requires be indicated");

                    Assert.That(Prose(outputDir, "ClassicDirect"), Does.Not.Contain("10.1021/acs.analchem.5b00140"),
                        "Classic deconvolution must not cite UniDec");
