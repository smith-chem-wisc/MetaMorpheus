...
namespace Test
{
    ...
    public static class GptmdPrunedDbTests
    {
        ...
        public static void TestPrunedDatabase()
        {
            ...
            //check Type, count, ID
            Assert.That("ConnorModType", Is.EqualTo(listOfMods[0].ModificationType));
            Assert.That("ConnorMod on P", Is.EqualTo(listOfMods[0].IdWithMotif));
            Assert.That(1, Is.EqualTo(listOfMods.Count));
            Directory.Delete(outputFolder, true);
            File.Delete(xmlName);
        ...
        public static void TestUserModSelectionInPrunedDB()
        {
            ...
            Assert.That(1, Is.EqualTo(Dlist[0].NumFixedMods));

            //check length
            Assert.That(3, Is.EqualTo(proteins[0].OneBasedPossibleLocalizedModifications.Count));
            ...
            //check Type, count, ID
            Assert.That("Common Fixed", Is.EqualTo(listOfLocalMods[0].ModificationType));
            Assert.That("missing", Is.EqualTo(listOfLocalMods[2].ModificationType));
            Assert.That(!listOfLocalMods.Contains(connorMod)); //make sure that mod set not to show up is not in mod list

            Assert.That("Default(Mod in DB and Observed) on P", Is.EqualTo(listOfLocalMods[0].IdWithMotif));
            Assert.That("ModToAlwaysAppear on P", Is.EqualTo(listOfLocalMods[1].IdWithMotif));
            //Makes sure Mod that was not in the DB but was observed is in pruned DB
            Assert.That("ModObservedNotinDB on E", Is.EqualTo(listOfLocalMods[2].IdWithMotif));
            Assert.That(3, Is.EqualTo(listOfLocalMods.Count));
            Directory.Delete(outputFolder, true);
            File.Delete(mzmlName);
            File.Delete(xmlName);
        ...
        public static void TestProteinPrunedWithModSelectionAndVariants()
        {
            ...
            // assert that mods on proteins are the same before/after task is run            
            Assert.That(proteinsLoaded.First().Accession, Is.EqualTo(protein1Variants.First().Accession));
            Assert.That(proteinsLoaded.First().OneBasedPossibleLocalizedModifications.Count(), Is.EqualTo(protein1Variants.First().OneBasedPossibleLocalizedModifications.Count()));
            Assert.That(proteinsLoaded.ElementAt(1).OneBasedPossibleLocalizedModifications.Count(), Is.EqualTo(protein2.OneBasedPossibleLocalizedModifications.Count()));
