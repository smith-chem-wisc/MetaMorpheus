...
namespace Test
{
    internal class CalibrationTests
    {
        ...
        public static void CalibrationTestLowRes()
        {
            ...
            string myFile = Path.Combine(TestContext.CurrentContext.TestDirectory, @"TestData\miniA549.mzML");
            ...
            Assert.That(File.Exists(Path.Combine(outputFolder, @"miniA549-calib.mzML")));
            Assert.That(File.Exists(Path.Combine(outputFolder, @"miniA549-calib.toml")));
            var lines = File.ReadAllLines(Path.Combine(outputFolder, @"miniA549-calib.toml"));
            var tolerance = Regex.Match(lines[0], @"\d+\.\d*").Value;
            var tolerance1 = Regex.Match(lines[1], @"\d+\.\d*").Value;
            Assert.That(double.TryParse(tolerance, out double tol) == true);
            Assert.That(double.TryParse(tolerance1, out double tol1) == true);
