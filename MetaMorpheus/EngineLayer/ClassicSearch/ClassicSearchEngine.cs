...
namespace EngineLayer.ClassicSearch
{
    public class ClassicSearchEngine : MetaMorpheusEngine
    {
        ...
        public ConcurrentDictionary<PeptideWithSetModifications, PeptideSpectralMatch> Dic_PeptideSpectralMatches;
        public List<PeptideSpectralMatch> PeptideSpectralMatches;
        private readonly PeptideWithSetModifications[] all_PeptideWithSetModifications;
        //private readonly PeptideWithMatchingScans[] PeptideWithMatchingScans;
        ...
        {
            PeptideSpectralMatches = globalPsms.ToList();
            ...
            all_PeptideWithSetModifications = new PeptideWithSetModifications[] { };
            Dic_PeptideSpectralMatches = new ConcurrentDictionary<PeptideWithSetModifications, PeptideSpectralMatch> { };
        ...
        public ClassicSearchEngine(List<PeptideSpectralMatch> globalPsms, Ms2ScanWithSpecificMass[] arrayOfSortedMS2Scans,
            List<Modification> variableModifications, List<Modification> fixedModifications, List<SilacLabel> silacLabels, SilacLabel startLabel, SilacLabel endLabel,
            List<Protein> proteinList, MassDiffAcceptor searchMode, CommonParameters commonParameters, List<(string FileName, CommonParameters Parameters)> fileSpecificParameters,
            SpectralLibrary spectralLibrary, List<string> nestedIds, bool writeSpectralLibrary,Dictionary<string,double> peptideFullSeq_HI)
            : base(commonParameters, fileSpecificParameters, nestedIds)
        {
        ...
        protected override MetaMorpheusEngineResults RunSpecific()
        {
            ...
            List<PeptideSpectralMatch> PeptideSpectralMatches_list = new List<PeptideSpectralMatch>();
            double proteinsSearched = 0;
            int oldPercentProgress = 0;

            // one lock for each MS2 scan; a scan can only be accessed by one thread at a time
            var myLocks = new object[PeptideSpectralMatches.Count];
                    ...
                    for (; i < Proteins.Count; i += maxThreadsPerFile)
                    {
                        ...
                        foreach (PeptideWithSetModifications peptide in Proteins[i].Digest(CommonParameters.DigestionParams, FixedModifications, VariableModifications, SilacLabels, TurnoverLabels))
                        {
                            ...
                            foreach (ScanWithIndexAndNotchInfo scan in GetAcceptableScans(peptide.MonoisotopicMass, SearchMode))
                            {
                                ...
                                AddScanCandidateToPeptide(peptide, myLocks, thisScore, scan, matchedIons);
            ...
            //foreach (var pep in Dic_PeptideSpectralMatches)
            //{
                PeptideSpectralMatches_list.AddRange(Dic_PeptideSpectralMatches.Values);
            //}
            PeptideSpectralMatches = PeptideSpectralMatches_list;
        ...
        //            //for (; i < Proteins.Count; i += maxThreadsPerFile)
        //            //{
        //            //    IEnumerable<PeptideWithSetModifications> peptides = Proteins[i].Digest(CommonParameters.DigestionParams, FixedModifications, VariableModifications, SilacLabels, TurnoverLabels);
        //            //    all_PeptideWithSetModifications.Add(peptides);
        //            //}
        //            for (; i < Proteins.Count; i += maxThreadsPerFile)
        //            {
        //                // Stop loop if canceled
        //                if (GlobalVariables.StopLoops) { return; }
        //                //for (int j=0;j< all_PeptideWithSetModifications.Length; j++)
        //                // digest each protein into peptides and search for each peptide in all spectra within precursor mass tolerance
        //                foreach (PeptideWithSetModifications peptide in Proteins[i].Digest(CommonParameters.DigestionParams, FixedModifications, VariableModifications, SilacLabels, TurnoverLabels))
        //                {
        //                    //var peptide = all_PeptideWithSetModifications[j];
        //                    PeptideWithSetModifications reversedOnTheFlyDecoy = null;
        ...
        //    foreach (var pep in Dic_PeptideSpectralMatches)
        //    {
        //        PeptideSpectralMatches_list.AddRange(pep.Value);
        //    }
        //    PeptideSpectralMatches = PeptideSpectralMatches_list;
        //    foreach (PeptideSpectralMatch psm in PeptideSpectralMatches.Where(p => p != null))
        //    {
        ...
        private void DecoyScoreForSpectralLibrarySearch(ScanWithIndexAndNotchInfo scan, PeptideWithSetModifications reversedOnTheFlyDecoy, Dictionary<DissociationType, List<Product>> decoyFragmentsForEachDissociationType, DissociationType dissociationType, object[] myLocks)
        {
            ...
            AddScanCandidateToPeptide(reversedOnTheFlyDecoy, myLocks, decoyScore,scan, decoyMatchedIons);
        ...
        private void AddScanCandidateToPeptide(PeptideWithSetModifications peptide, object[] myLocks, double thisScore, ScanWithIndexAndNotchInfo scan, List<MatchedFragmentIon> matchedIons)
        {
            if (thisScore>3)
            {
                PeptideSpectralMatch psm = new PeptideSpectralMatch(peptide, scan.Notch, thisScore, scan.ScanIndex, scan.TheScan, CommonParameters, matchedIons, 0);
                if (!Dic_PeptideSpectralMatches.ContainsKey(peptide))
                {
                    Dic_PeptideSpectralMatches.TryAdd(peptide,  psm );
                }
                //else if(Dic_PeptideSpectralMatches[peptide].Count<5)
                //{
                //    Dic_PeptideSpectralMatches[peptide].Add(psm);
                //}
                //else if(thisScore> Dic_PeptideSpectralMatches[peptide].Select(p=>p.Score).ToList().Min())
                else if (thisScore > Dic_PeptideSpectralMatches[peptide].Score)
                {
                    var old_psm = Dic_PeptideSpectralMatches[peptide];
                    Dic_PeptideSpectralMatches.TryRemove(peptide,out old_psm);
                    Dic_PeptideSpectralMatches.TryAdd(peptide, psm);
                    //double min = Dic_PeptideSpectralMatches[peptide].Select(p => p.Score).ToList().Min();
                    //int index = Dic_PeptideSpectralMatches[peptide].Select(p => p.Score).ToList().IndexOf(min);
                    //Dic_PeptideSpectralMatches[peptide].Remove(Dic_PeptideSpectralMatches[peptide][index]);
                    //Dic_PeptideSpectralMatches[peptide].Add(psm);
                }

            }
            //List<PeptideMatchingScans> PeptideMatchingScans
            //bool meetsScoreCutoff = thisScore >= CommonParameters.ScoreCutoff;
            ...
            //lock (myLocks[scan.ScanIndex])
            //{
            //bool scoreImprovement = PeptideMatchingScans[scan.ScanIndex] == null || (thisScore - PeptideSpectralMatches[scan.ScanIndex].RunnerUpScore) > -PeptideSpectralMatch.ToleranceForScoreDifferentiation;
