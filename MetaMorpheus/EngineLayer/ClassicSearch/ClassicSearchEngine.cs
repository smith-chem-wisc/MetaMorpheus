...
namespace EngineLayer.ClassicSearch
{
    public class ClassicSearchEngine : MetaMorpheusEngine
    {
        ...
        /// <summary>Per-scan candidate score distributions, null unless Tailor scoring was requested.</summary>
        private readonly ScoreDistribution[] CandidateScoreDistributions;
        ...
        public ClassicSearchEngine(SpectralMatch[] globalPsms, Ms2ScanWithSpecificMass[] arrayOfSortedMS2Scans,
            ...
            bool computeTailorScore = false)
            : base(commonParameters, fileSpecificParameters, nestedIds)
        {
            SpectralMatches = globalPsms;
            ArrayOfSortedMS2Scans = arrayOfSortedMS2Scans;
            // The search mode decides which observed mass candidates are selected with (monoisotopic, or
            // the most-abundant isotopologue). The caller sorted the scans by that same quantity, which is
            // what keeps the binary search below valid.
            MyScanPrecursorMasses = arrayOfSortedMS2Scans.Select(b => b.GetPrecursorMassForSearch(commonParameters)).ToArray();
            ...
            // Allocated only on request: one histogram per scan is a fixed but not free cost.
            if (computeTailorScore)
            {
                CandidateScoreDistributions = new ScoreDistribution[SpectralMatches.Length];
                for (int i = 0; i < CandidateScoreDistributions.Length; i++)
                {
                    CandidateScoreDistributions[i] = new ScoreDistribution();
                }
            }
        ...
        protected override MetaMorpheusEngineResults RunSpecific()
        {
           Status("Getting ms2 scans...");
                    ...
                    for (int i = range.Item1; i < range.Item2; i++)
                    {
                        ...
                        foreach (var specificBioPolymer in Proteins[i].Digest(CommonParameters.DigestionParams, FixedModifications, VariableModifications, SilacLabels, TurnoverLabels))
                        {
                            ...
                            foreach (ScanWithIndexAndNotchInfo scan in GetAcceptableScans(specificBioPolymer.MonoisotopicMass, SearchMode))
                            {
                                ...
                                // Recorded before the score cutoff: Tailor calibrates against the whole
                                // candidate distribution, most of which never becomes a PSM.
                                CandidateScoreDistributions?[scan.ScanIndex].Add(thisScore);
            ...
            if (CandidateScoreDistributions != null)
            {
                for (int i = 0; i < SpectralMatches.Length; i++)
                {
                    if (SpectralMatches[i] != null)
                    {
                        SpectralMatches[i].TailorScore = TailorScoreCalculator.Calculate(SpectralMatches[i].Score, CandidateScoreDistributions[i]);
                    }
                }
            }
        ...
        private void DecoyScoreForSpectralLibrarySearch(ScanWithIndexAndNotchInfo scan, IBioPolymerWithSetMods reversedOnTheFlyDecoy, Dictionary<DissociationType, List<Product>> decoyFragmentsForEachDissociationType, DissociationType dissociationType)
        {
            ...
            // In library search the protein-level decoys are skipped and reversed decoys are made here
            // instead, so leaving them out would halve the candidate distribution.
            CandidateScoreDistributions?[scan.ScanIndex].Add(decoyScore);
