...
namespace EngineLayer
{
    ...
    /// in the IsobaricMassTagReporterIonIntensities property on Ms2ScanWithSpecificMass and SpectralMatch.
    /// </summary>
    public class IsobaricMassTag
    {
        ...
        public static List<string> GetReporterIonLabels(IsobaricMassTagType tagType)
        {
            return tagType switch
            {
                ...
                // iTRAQ 8-plex has no 120 channel: it is skipped because the phenylalanine immonium ion
                // sits at 120.081, and the eighth reagent is 121. The m/z extracted here has always been
                // the right one -- H12 C{13}6 N{15}2 plus a proton is 121.1215, against a published
                // 121.122 -- so only the name was wrong. The "# 120" comment on that DI line in
                // Mods/tmt.txt is where it came from.
