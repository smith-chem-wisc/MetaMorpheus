...
namespace GoodGUI
{
    internal class GPTMD
    {
        private const double tol = 0.0085;
        internal EventHandler<string> labelStatusHandler;
        internal EventHandler<int> progressHandler;
        internal EventHandler<string> outputHandler;
        ...
        private static IEnumerable<MorpheusModification> GetMod(double massDiff, bool isotopeErrors, IEnumerable<MorpheusModification> allMods, IEnumerable<Tuple<double, double>> combos)
        {
            foreach (var Mod in allMods)
            {
                ...
                if (isotopeErrors && Mod.MonoisotopicMassShift > massDiff - tol - 1.003 && Mod.MonoisotopicMassShift < massDiff + tol - 1.003)
                    yield return Mod;
                if (Mod.AlternativeMassShift != double.NaN && Mod.AlternativeMassShift > massDiff - tol && Mod.AlternativeMassShift < massDiff + tol)
                    yield return Mod;
                if (Mod.AlternativeMassShift != double.NaN && isotopeErrors && Mod.AlternativeMassShift > massDiff - tol - 1.003 && Mod.AlternativeMassShift < massDiff + tol - 1.003)
                    yield return Mod;
            }
            ...
            foreach (var combo in combos)
            {
                ...
                {
                    foreach (var mod in GetMod(m1, isotopeErrors, allMods, combos))
                        yield return mod;
                    foreach (var mod in GetMod(m2, isotopeErrors, allMods, combos))
                        yield return mod;
                }
