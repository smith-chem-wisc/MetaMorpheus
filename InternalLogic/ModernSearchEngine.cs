...
namespace InternalLogicEngineLayer
{
	public class ModernSearchEngine : MyEngine
	{
		...
		readonly List<int>[] fragmentIndex;

		readonly double fragmentToleranceInDaltons;

		readonly float[] keys;

		readonly IMsDataFile<IMzSpectrum<MzPeak>> myMsDataFile;

		readonly List<CompactPeptide> peptideIndex;

		readonly List<SearchMode> searchModes;

		readonly int spectraFileIndex;
		...
		static bool FirstIsPreferableWithoutScore(CompactPeptide first, CompactPeptide second, double pm)
		{
		...
		void CalculatePeptideScores(IMsDataScan<IMzSpectrum<MzPeak>> spectrum, double[] peptideScores)
		{
