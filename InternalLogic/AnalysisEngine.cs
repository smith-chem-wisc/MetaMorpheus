...
namespace InternalLogicEngineLayer
{
	public class AnalysisEngine : MyEngine
	{
		...
		readonly ParentSpectrumMatch[][] newPsms;

		readonly Dictionary<CompactPeptide, HashSet<PeptideWithSetModifications>> compactPeptideToProteinPeptideMatching;
		...
		readonly List<SearchMode> searchModes;

		readonly IMsDataFile<IMzSpectrum<MzPeak>> myMsDataFile;

		readonly Tolerance fragmentTolerance;

		readonly Action<BinTreeStructure, string> action1;

		readonly Action<List<NewPsmWithFDR>, string> action2;

		readonly bool doParsimony;
		...
		public Dictionary<CompactPeptide, HashSet<PeptideWithSetModifications>> ApplyProteinParsimony(Dictionary<CompactPeptide, HashSet<PeptideWithSetModifications>> fullSequenceToProteinPeptideMatching)
		{
			...
			var newDict = new Dictionary<Protein, HashSet<CompactPeptide>>();
			...
			var usedPeptides = new HashSet<CompactPeptide>();
			var bestProteinPeptideList = new HashSet<CompactPeptide>();
			...
			while (currentBestNumNewPeptides != 0)
			{
				...
				if (bestProteinHasOnePeptide == false)
				{
					...
					bestProteinHasOnePeptide |= currentBestNumNewPeptides == 1;
			...
			var peptideProteinGroupMatch = new Dictionary<CompactPeptide, HashSet<Protein>>();
			...
			foreach (var kvp in parsimonyDict)
			{
				foreach (var peptide in kvp.Value)
				{
					...
					{
						var proteinGroup = new HashSet<Protein>();
			...
			foreach (var kvp in parsimonyDict)
			{
				foreach (var peptide in kvp.Value)
				{
					...
					{
						var oldVirtualPeptides = new HashSet<PeptideWithSetModifications>();
						var newVirtualPeptides = new HashSet<PeptideWithSetModifications>();
						var proteinGroup = new HashSet<Protein>();
		...
		static void IdentifyAAsInCommon(BinTreeStructure myTreeStructure)
		{
		...
		static void IdentifyMods(BinTreeStructure myTreeStructure)
		{
			foreach (Bin bin in myTreeStructure.finalBins)
			{
		...
		static void IdentifyResidues(BinTreeStructure myTreeStructure)
		{
			foreach (Bin bin in myTreeStructure.finalBins)
			{
		...
		static void IdentifyUnimodBins(BinTreeStructure myTreeStructure, double v)
		{
		...
		static void IdentifyUniprotBins(BinTreeStructure myTreeStructure, double v)
		{
			foreach (var bin in myTreeStructure.finalBins)
			{
				var ok = new HashSet<string>();
		...
		static void IdentifyCombos(BinTreeStructure myTreeStructure, double v)
		{
		...
		static void IdentifyAA(BinTreeStructure myTreeStructure, double v)
		{
			foreach (var bin in myTreeStructure.finalBins)
			{
				var ok = new HashSet<string>();
		...
		static void IdentifyMine(BinTreeStructure myTreeStructure, double v)
		{
		...
		static BinTreeStructure MyAnalysis(List<NewPsmWithFDR> limitedpsms_with_fdr)
		{
		...
		static List<NewPsmWithFDR> DoFalseDiscoveryRateAnalysis(IEnumerable<PSMwithTargetDecoyKnown> items)
		{
		...
		List<ProteinGroup> BuildProteinGroupsAndDoProteinFDR(List<NewPsmWithFDR> psmList)
		{
			var proteinGroups = new List<ProteinGroup>();

			var dictionary1 = new Dictionary<Protein, List<NewPsmWithFDR>>();
			var dictionary2 = new Dictionary<NewPsmWithFDR, HashSet<Protein>>();

			var proteinGroupProteinList = new HashSet<Protein>();
			...
			foreach (var psm in psmList)
			{
				...
				var virtualPeptides = new HashSet<PeptideWithSetModifications>();
		...
		void AddObservedPeptidesToDictionary()
		{
